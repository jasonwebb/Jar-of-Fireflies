/*
  Jar of Fireflies with external analog sensor
...
// Sensor variables
const int sensorBufferSize = 10;
int sensorValues[sensorBufferSize];
...
void checkSensor() {
  // Fill up the buffer with readings
  if (sensorCounter < sensorBufferSize) {
    int val = analogRead(sensorPin);
    sensorValues[sensorCounter] = val;
    sensorCounter++;
    return;
  }
  ...
  avg = avg / sensorBufferSize;

  // If the average is above the threshold, initiate "dance mode" state
  if (avg >= sensorThreshold) {
    CURRENT_ACTION = DANCE;
  }
